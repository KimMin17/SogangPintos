...
process_execute (const char *file_name) 
{
  ...
  //exec-missing check
	if(filesys_open(exe_name) == NULL) return -1;
  ...
  for(e = list_begin(&thread_current()->child_list); e != list_end(&thread_current()->child_list); e = list_next(e)) {
    ...
    if(t->load_check == -1) return process_wait(tid);
  }
  return tid;
}
...
start_process (void *file_name_)
{
  ...
  else thread_current()->load_check = 1;
...
void push_arg(const char* file_name, void** esp) {
	...
	token = strtok_r(tmp_file_name, " ", &next_ptr);
	while(token != NULL) {
		argc++;
  ...
  arg_addr = (uint32_t*)malloc(sizeof(uint32_t) * argc);
